...
Following [Prometheus](https://github.com/nholuongut/prometheus/blob/main/RELEASE.md) and [Thanos](https://github.com/thanos-io/thanos/blob/main/docs/release-process.md), this project aims for a predictable release schedule.
...
If any of the maintainers is interested in volunteering please create a pull request against the [nholuongut/prometheus-operator](https://github.com/nholuongut/prometheus-operator) repository and propose yourself for the release series of your choice.
...
> This guide is strongly based on the [Prometheus release instructions](https://github.com/nholuongut/prometheus/blob/main/RELEASE.md).
...
A couple of days before the release, update the [default versions](https://github.com/nholuongut/prometheus-operator/blob/f6ce472ecd6064fb6769e306b55b149dfb6af903/pkg/operator/defaults.go#L20-L31) of Prometheus, Alertmanager and Thanos if newer versions are available.
...
```bash
go mod edit -require "github.com/nholuongut/prometheus-operator/pkg/apis/monitoring@v$(< VERSION)" pkg/client/go.mod
go mod edit -require "github.com/nholuongut/prometheus-operator/pkg/apis/monitoring@v$(< VERSION)"
go mod edit -require "github.com/nholuongut/prometheus-operator/pkg/client@v$(< VERSION)"
```
...
For example: https://github.com/nholuongut/prometheus-operator/compare/v0.72.0...release-0.73
...
**Note:** The remote name `origin` is assumed to be pointed to `github.com/nholuongut/prometheus-operator`. If you have a different remote name, use that instead of `origin`. Verify this using `git remote -v`.
...
Tag the new release with a tag named `v<major>.<minor>.<patch>`, e.g. `v2.1.3`. Note the `v` prefix. Tag also the `github.com/nholuongut/prometheus-operator/pkg/apis/monitoring` module with `pkg/apis/monitoring/v<major>.<minor>.<patch>` and the `github.com/nholuongut/prometheus-operator/pkg/client` module with `pkg/client/v<major>.<minor>.<patch>`. You can do the tagging on the commandline:
...
Once a tag is created, the `publish` Github action will push the container images to [quay.io](https://quay.io/organization/prometheus-operator) and [ghcr.io](https://github.com/nholuongut/prometheus-operator/pkgs/container/prometheus-operator). Wait until the [publish](https://github.com/nholuongut/prometheus-operator/actions/workflows/publish.yaml) workflow is complete before going to the next step.
...
Go to https://github.com/nholuongut/prometheus-operator/releases/new, associate the new release with the before pushed tag, paste in changes made to `CHANGELOG.md` and click "Publish release".

Once release is published, [release job](https://github.com/nholuongut/prometheus-operator/actions/workflows/release.yaml) will be triggered to upload assets to the newly created release.

For patch releases, submit a pull request to merge back the release branch into the `main` branch.

## Update website

Bump the operator's version in the [website](https://github.com/nholuongut/website/blob/main/data/prometheusOperator.json) repository.
