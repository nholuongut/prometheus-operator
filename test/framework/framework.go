...
import (
	...
	v1monitoringclient "github.com/nholuongut/prometheus-operator/pkg/client/versioned/typed/monitoring/v1"
	v1alpha1monitoringclient "github.com/nholuongut/prometheus-operator/pkg/client/versioned/typed/monitoring/v1alpha1"
	v1beta1monitoringclient "github.com/nholuongut/prometheus-operator/pkg/client/versioned/typed/monitoring/v1beta1"
)
...
func (f *Framework) CreateOrUpdatePrometheusOperatorWithOpts(
	...
	_, err := f.createOrUpdateServiceAccount(
		ctx,
		opts.Namespace,
		fmt.Sprintf("%s/rbac/nholuongut/prometheus-operator-service-account.yaml", f.exampleDir),
	)
	if err != nil {
		return nil, fmt.Errorf("failed to create or update prometheus operator service account: %w", err)
	}

	clusterRole, err := f.CreateOrUpdateClusterRole(ctx, fmt.Sprintf("%s/rbac/nholuongut/prometheus-operator-cluster-role.yaml", f.exampleDir))
	if err != nil {
	...
	if opts.ClusterRoleBindings {
		...
		if _, err := f.createOrUpdateClusterRoleBinding(ctx, opts.Namespace, fmt.Sprintf("%s/rbac/nholuongut/prometheus-operator-cluster-role-binding.yaml", f.exampleDir)); err != nil {
	...
	deploy, err := MakeDeployment(fmt.Sprintf("%s/rbac/nholuongut/prometheus-operator-deployment.yaml", f.exampleDir))
	if err != nil {
		return nil, err
	}
	...
	service, err := MakeService(fmt.Sprintf("%s/rbac/nholuongut/prometheus-operator-service.yaml", f.exampleDir))
	if err != nil {
...
func (f *Framework) DeletePrometheusOperatorClusterResource(ctx context.Context) error {
	err := f.DeleteClusterRole(ctx, fmt.Sprintf("%s/rbac/nholuongut/prometheus-operator-cluster-role.yaml", f.exampleDir))
